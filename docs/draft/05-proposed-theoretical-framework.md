...
The proposed framework is structured as a network of interlinked concepts forming a plane of understanding. The three dimensions (TECH, PED, INST) are not isolated variables but mutually articulating concepts that shape platform success. This framework is grounded in the DeLone & McLean model (2003)~[b31], which states that system success requires system, information, and service qualities. In this domain, they are operationalised as: TECH $\leftarrow$ System Quality, PED $\leftarrow$ Information Quality, INST $\leftarrow$ Service Quality. This operationalisation is not merely a relabeling; each requires conceptual enrichment from domain-specific literature (CALL, SLA, EFLA) because D&M does not define domain-specific metrics. This network is further enriched by crossing concepts from TAM (Davis, 1989)~[b11] and Self-Determination Theory (SDT) (Deci & Ryan, 1985)~[b12] to bridge platform qualities and outcomes.
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{} L{3.5cm} L{3.8cm} L{8.0cm} @{} }
    ...
    **Multi-Stakeholder Perspective** & Scheffel et al. (2014)~[b9] & Integrates the views of learners, instructors, and administrators; operationalised in INST-3 and INST-4. 
    \bottomrule
  \end{tabular}
\end{table*}
